...
{
	node* t_0; //4 узла - составл€ющие 2 дуг,
	node* t_1; //между которыми вставл€ютс€ ретикул€ции
	node* t_2;
	node* t_3;
	node* ret_0 = new node(); //2 узла - составл€ющие ретикул€ции, 
	node* ret_1 = new node(); //которые вставл€ютс€ посередине исходных дуг

	t_0 = get_to_node(tree, edg_1[0]); //Ќачало 1й дуги
	t_1 = get_to_node(tree, edg_2[0]); //Ќачало 2й дуги

	t_2 = get_to_node(tree, edg_1[1]); // онец 1й дуги
	t_3 = get_to_node(tree, edg_2[1]); // онец 2й дуги
